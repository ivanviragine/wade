"""End-to-end workflow chain tests via the wade CLI binary.

Tests the full wade lifecycle by invoking `wade` as a subprocess,
...
Requires `wade` to be installed (e.g., `uv pip install -e .`).
"""
...
def require_wade() -> None:
    """Skip all tests if wade is not installed."""
...
def e2e_repo(tmp_path: Path) -> Path:
    ...
    Simulates what a user would have after running `wade init`.
    """
...
def mock_gh_cli(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, Any]:
    ...
    Prepends the mock binary directory to PATH so wade's subprocess
...
"""
    ...
    # Prepend mock bin to PATH so wade subprocess calls hit the mock
...
class TestCLIBasics:
    """Smoke tests for basic wade commands."""

    def test_version(self) -> None:
        """wade --version exits 0 and prints version string."""
        result = subprocess.run([WADE, "--version"], capture_output=True, text=True, timeout=10)
        assert result.returncode == 0
        assert "wade" in result.stdout.lower()

    def test_help(self) -> None:
        """wade --help exits 0 and lists key subcommands."""
        result = subprocess.run([WADE, "--help"], capture_output=True, text=True, timeout=10)
        assert result.returncode == 0
