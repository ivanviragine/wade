...
from wade.models.session import MergeStrategy, SyncResult
from wade.services.implementation_service import _done_via_direct
...
_BASE_PATCH_TARGETS: dict[str, dict] = {
    "wade.services.implementation_service.done.get_provider": {},
    "wade.services.implementation_service.done.git_sync.fetch_origin": {},
    "wade.services.implementation_service.done.git_sync.merge_branch": {"return_value": _SYNC_OK},
    "wade.services.implementation_service.done.git_repo._run_git": {},
    "wade.services.implementation_service.done.remove_in_progress_label": {},
    "wade.services.implementation_service.done.console": {},
}
...
class TestDoneViaDirectCleanup:
    ...
    def test_cleanup_success_no_prompt(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch("wade.services.implementation_service.done.git_branch.delete_branch")
            )
            stack.enter_context(
                patch("wade.services.implementation_service.done.git_worktree.prune_worktrees")
            )
            mock_select = stack.enter_context(
                patch("wade.services.implementation_service.done.prompts.select")
            )
    ...
    def test_cleanup_failure_prompts_user(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "wade.services.implementation_service.done.git_branch.delete_branch",
                    side_effect=error,
                )
            )
            stack.enter_context(
            ...
            mock_select = stack.enter_context(
                patch(
                    "wade.services.implementation_service.done.prompts.select",
                    return_value=1,
                )
            )
            stack.enter_context(patch("wade.services.implementation_service.done.logger"))
    ...
    def test_cleanup_failure_retry_succeeds(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            mock_logger = stack.enter_context(
                patch("wade.services.implementation_service.done.logger")
            )
    ...
    def test_worktree_path_uses_cleanup_worktree(self) -> None:
        ...
        with ExitStack() as stack:
            ...
            mock_cleanup = stack.enter_context(
                patch("wade.services.implementation_service.done._cleanup_worktree")
            )
            mock_delete = stack.enter_context(
                patch("wade.services.implementation_service.done.git_branch.delete_branch")
            )
