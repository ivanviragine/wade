...
from wade.services import work_service
...
def test_resolve_plan_extracts_title(tmp_path: Path) -> None:
    ...
    with (
        patch("wade.services.work_service.find_worktree_path", return_value=expected_path),
        patch(
            "wade.services.work_service.git_repo.get_current_branch",
...
def test_resolve_plan_finds_worktree_by_slug(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "wade.services.work_service.find_worktree_path", return_value=expected_path
        ) as mock_find,
        patch(
            "wade.services.work_service.git_repo.get_current_branch",
...
def test_done_with_plan_flag_resolves_and_delegates() -> None:
    ...
    with (
        ...
        patch("wade.services.work_service.get_provider", return_value=provider),
        patch("wade.services.work_service.git_repo.get_repo_root", return_value=Path("/tmp/repo")),
        patch(
            "wade.services.work_service._resolve_worktree_from_plan",
        ...
        patch("wade.services.work_service._done_via_pr", return_value=True) as mock_done,
    ):
...
def test_done_plan_flag_error_returns_false() -> None:
    ...
    with (
        ...
        patch("wade.services.work_service.get_provider", return_value=MagicMock()),
        patch("wade.services.work_service.git_repo.get_repo_root", return_value=Path("/tmp/repo")),
        patch(
            "wade.services.work_service._resolve_worktree_from_plan",
