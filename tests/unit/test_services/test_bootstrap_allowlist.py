...
class TestBootstrapPlanMode:
    ...
    def test_codex_hooks_feature_flag_enabled(self, tmp_path: Path) -> None:
        ...
        crossby #136 (v0.23.5) migrated the deprecated ``[features].codex_hooks``
        alias to the canonical ``[features].hooks = true`` and removes the old
        key. This asserts the post-migration shape.
        """
        import tomllib
        ...
        # The deprecated alias must not linger alongside the canonical key.
