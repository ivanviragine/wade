...
Retargeting a PR's base (gh pr edit --base / update_pr_base) does NOT rewrite the head branch's ancestry: a scaffold branch cut from the OLD base keeps that base's commits, which then land in the NEW base's diff and merge into it (the startup catchup merge cannot remove them). draft_pr.reroot_scaffold_branch_for_retarget (shared by the plan and implement retarget paths, called before update_pr_base) rebuilds a scaffold-ONLY branch on the new base via git_branch.reset_branch + create_scaffold_commit + force-push; it leaves branches with real work (a worktree, or >1 commit past the old base) untouched. is_valid_git_ref must special-case the reserved whole ref 'HEAD' (git check-ref-format --branch rejects it) even though 'feat/HEAD' and 'head' are valid.
