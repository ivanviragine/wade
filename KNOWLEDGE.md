...
conventional_title_error() (utils/conventional.py) must NOT escape/sanitize the title it echoes — conventional.py is stdlib-only by contract (wade-hook Stop cold-start), so no rich import even lazily. Instead sanitize untrusted titles at the RENDER boundary: console.error()/console.detail()/console.success() take a keyword-only markup=False (added #392) that shows bracket tokens like a stray [/] literally while keeping the prefix/styling, avoiding the MarkupError crash (see 66c7e8b329cc). Every site that echoes a provider-derived title must pass markup=False — this includes SUCCESS lines, not just errors: task_service.create_interactive, done._gate_pr_title, done._done_via_pr's PR-title-sync success line, cli/task + core + done plan-file ConventionalTitleError handlers, plan_service._select_valid_plans, cli/plan_session.done. The final 'Implementation done' panel (done.py, via console.issue_ref) also renders the title through Rich but predates #392 and is out of scope here.
