...
### Finding
The `_slugify()` function in `src/ghaiw/git/branch.py` already uses `-` (dash) for non-ASCII character replacement, not `?`. The implementation is correct:
...
- ✅ `uv run mypy src/ghaiw/git/branch.py --strict` → 0 errors
- ✅ `uv run ruff check src/ghaiw/git/branch.py tests/unit/test_git/test_branch_slugify.py` → 0 errors
...
### Implementation
- Added `import warnings` to `src/ghaiw/ai_tools/base.py` (line 11, alphabetically ordered)
...
- `uv run ruff check src/ghaiw/ai_tools/base.py` → All checks passed
...
- Patch `ghaiw.services.work_service.logger` as a whole MagicMock to assert `mock_logger.warning.assert_called_once_with(...)`
...
- `prompts.select` is in `ghaiw.ui.prompts`; import as `from ghaiw.ui import prompts` (added alphabetically before `from ghaiw.ui.console import console`)
