Planning phase in a detached worktree (no PLAN.md at the root; plans go under the plan directory). First action: run `wade plan-session check`; proceed only when it reports `IN_WORKTREE`.
