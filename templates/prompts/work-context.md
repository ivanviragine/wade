...
1. **First action**: run `ghaiw check` to verify you're in a worktree.
2. **Immediately**: create `/tmp/PR-SUMMARY-{issue_number}.md` as a placeholder.
3. **Never** push branches or create PRs manually — use `ghaiw work done`.
4. **Before closing**: sync with main via `ghaiw work sync --json`.
