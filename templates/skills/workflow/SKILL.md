...
Before running any `ghaiw` command, ensure it runs with the required
...
Do not "try sandbox first" for `ghaiw`/`gh`; run them unsandboxed from the
start.

If a `ghaiw` command fails due to auth, permission, lock-file, or similar
...
- `IN_MAIN_CHECKOUT` → planning and issue operations are allowed; **editing any
  ...
  a worktree first via `ghaiw work start`.
...
plans, and running `ghaiw task create` are all safe from the main checkout.
...
2. Create the issue via `ghaiw task create --plan-file`
...
> **Exception — inside a `ghaiw task plan` session:** If you were launched by
> `ghaiw task plan`, do **not** apply Rule 4. Your job is to write plan
...
> when planning outside of a `ghaiw task plan` session.

## Rule 5: Closing the session — use `ghaiw work done`
...
1. **Write a PR summary** — Create `/tmp/PR-SUMMARY-{issue-number}.md` (e.g. `/tmp/PR-SUMMARY-42.md`)
   ...
   `ghaiw work done` reads this file to populate the PR body. Without it the PR description will be empty.
   Write outside the repo (to `/tmp`).
2. Run `ghaiw work sync` to sync with main.
3. Run `ghaiw work done`. This command handles:
   - Pushing the branch
   - Creating the PR (or merging, based on config)
   - **Triggering auto-versioning** (if enabled)
4. After `ghaiw work done` completes, you can exit your session normally. The
...
- **Interactively**: `ghaiw task create`
...
## Rule 7: Do not run `ghaiw` or `gh` commands in sandboxed mode

If your environment supports sandboxed command execution, run `ghaiw`/`gh`
commands with the required permissions/capabilities instead of sandbox mode.

Do not attempt sandbox execution first. If a `ghaiw`/`gh` command fails with
