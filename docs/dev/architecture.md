...
**Repo-quality gates** (`HooksConfig` → `PreCommitConfig` / `CommitMsgConfig` / `PostToolUseConfig`, #352): three opt-in, off-by-default subsections. `pre_commit.{lint,test}` and `commit_msg.conventional` install per-worktree `pre-commit` / `commit-msg` git hooks (baked from config at bootstrap via placeholder substitution — no per-commit config load), installed together with the `done.pre_push_backstop` `pre-push` hook through the single batch `install_worktree_git_hooks` call so every prior user hook is captured **before** wade sets `core.hooksPath` (per-hook `.chain-<hook_name>` files; a #349 unsuffixed `.chain` is migrated to `.chain-pre-push` on upgrade). `post_tool_use` installs a PostToolUse hook (`wade-hook post_tool_use …`, fail-open, never blocks) into context-capable tools only (dialect ≠ `DECISION`, so Antigravity CLI is skipped) that lints the just-edited file (`lint_cmd` file-scoped; falls back to `pre_commit.lint` whole-repo) and injects findings back as `additionalContext`. All three, like `done`, derive their `check_service` validator allowlists from `*.model_fields` so config-key validity can't drift (knowledge `ca245d6a`). **Honesty:** `git commit --no-verify` bypasses the git hooks — these are quality gates, not enforcement boundaries.
