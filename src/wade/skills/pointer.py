...
(or CLAUDE.md) that directs AI agents to the wade workflow skill.
...
from wade.utils.markdown import extract_marker_block, has_marker_block

logger = structlog.get_logger()

MARKER_START = "<!-- wade:pointer:start -->"
MARKER_END = "<!-- wade:pointer:end -->"
...
def has_pointer(file_path: Path) -> bool:
    """Check if a file contains the wade pointer (marker-based)."""
    ...
    if count > 1:
        logger.warning(
            ...
            msg=(
                ...
                "This is expected in the wade repo itself "
